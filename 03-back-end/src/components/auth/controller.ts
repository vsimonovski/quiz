...
export default class AuthController extends BaseController {
  public async userLogin(req: Request, res: Response) {
    ...
    if (!(user instanceof UserModel)) {
      return res
        .status(404)
        .send({ errorCode: user.errorCode, errorMessage: user.errorMessage });
    }
    ...
    if (!(await bcrypt.compare(data.password, user.passwordHash))) {
      ...
      return res
        .status(403)
        .send({ errorCode: 403, errorMessage: "Invalid user password." });
    }
    ...
    const authToken = jwt.sign(authTokenData, Config.auth.user.auth.private, {
